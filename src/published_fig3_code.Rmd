...
```{r}
Tcells_analyzed = readRDS(file = '~/Downloads/Tcells.rds')
Tcells = Tcells_analyzed
...
#making also junk object lmao
Tcells.junk <- subset(Tcells, subset = nFeature_RNA < 750)
Tcells.junk <- subset(Tcells.junk, subset = nCount_RNA < 1500)
Tcells.junk <- subset(Tcells.junk, subset = percent.mito > 5)
Tcells.junk
```

# 3. Import Demuxlet information into Seurat.

The data was generated with three separate 10X lanes, labeled here as CD4, CD4-8, and CD8

```{r}
Tcells.cut <- importDemux(
  Tcells.cut,
  demuxlet.best = c("~/Downloads/Tcells/Demuxlet/CD4.best",
                    "~/Downloads/Tcells/Demuxlet/CD4-8.best",
                    "~/Downloads/Tcells/Demuxlet/CD8.best"),
...
Tcells.cut[["Sample"]] <- sapply(
    ...
    function(X) strsplit(X, split = "CD4_")[[1]][2])
```
(code as downloaded filtering)
The average number of SNPs per cell for all lanes was: 495
Out of 46787 cells in the Seurat object, Demuxlet assigned:
    40153 cells or 85.8% as singlets
    6634 cells or 14.2% as doublets
    and 0 cells as too ambiguous to call.
0 cells were not annotated in the demuxlet.best file.

(without filtering)
The average number of SNPs per cell for all lanes was: 481.9
Out of 2211840 cells in the Seurat object, Demuxlet assigned:
    42793 cells or 1.9% as singlets
    6866 cells or 0.3% as doublets
    and 0 cells as too ambiguous to call.
2162181 cells were not annotated in the demuxlet.best file.
...
# exp.mat <- read.table(file = "~/Downloads/Tcells/cell_cycle_vignette_files/nestorawa_forcellcycle_expressionMatrix.txt",
#     header = TRUE, 
#     as.is = TRUE,
#     row.names = 1)
